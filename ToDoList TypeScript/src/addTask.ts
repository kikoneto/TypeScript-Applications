...
if (addTaskButton) {
    addTaskButton.addEventListener('click', (e) => {
        let titleParent = (<HTMLElement>e.currentTarget).parentNode!.childNodes[3].childNodes[1] as HTMLInputElement;
        let infoParent = (<HTMLElement>e.currentTarget).parentNode!.childNodes[3].childNodes[3] as HTMLInputElement;

        let title: string = titleParent.value;
        let info: string = infoParent.value;
        ...
        if (title !== '') {
            ...
            if (newTask.childNodes.length == 2) {
                activateCheckButton(newTask.childNodes[1].childNodes[0] as HTMLElement);
                activateDeleteButton(newTask.childNodes[1].childNodes[1] as HTMLElement);
            } else if (newTask.childNodes.length == 3) {
                activateCheckButton(newTask.childNodes[2].childNodes[0] as HTMLElement);
                activateDeleteButton(newTask.childNodes[2].childNodes[1] as HTMLElement);
            }
